...
public class PACEHandler {
    ...
    public init(cardAccess : CardAccess, tagReader: TagReader) throws {
        ...
        guard let paceInfo: PACEInfo = cardAccess.paceInfo else {
            Crashlytics.crashlytics().setCustomValue("not supported", forKey: FirebaseCustomKeys.PACEStatus)
        ...
        do {
            if let paceInfo = paceInfos.removeFirst() {
                ...
                Crashlytics.crashlytics().setCustomKeysAndValues([
                    ...
                    FirebaseCustomKeys.PACEInfoVersion: paceInfo.version,
                    FirebaseCustomKeys.PACEInfoOID: paceOID
                ])
...
*/
    ...
    func doStep1() {
        Crashlytics.crashlytics().setCustomValue("Step 1", forKey: FirebaseCustomKeys.PACEStep)
    ...
    func doStep2( passportNonce: [UInt8]) {
        ...
        Crashlytics.crashlytics().setCustomValue("Step 2", forKey: FirebaseCustomKeys.PACEStep)
        switch(mappingType) {
            case .CAM, .GM:
    ...
    func doPACEStep2GM(passportNonce : [UInt8]) {
        Crashlytics.crashlytics().setCustomValue("Step 2 GM", forKey: FirebaseCustomKeys.PACEStep)
    ...
    func doStep3KeyExchange(ephemeralParams: OpaquePointer) {
            Crashlytics.crashlytics().setCustomValue("Step 3 Key exchange", forKey: FirebaseCustomKeys.PACEStep)
    ...
    func doStep3KeyAgreement( pcdKeyPair: OpaquePointer, passportPublicKey: OpaquePointer) {
        Crashlytics.crashlytics().setCustomValue("Step 3 Key agreement", forKey: FirebaseCustomKeys.PACEStep)
        ...
        tagReader.sendGeneralAuthenticate(data:step4Data, isLast:true, completed: { [weak self] response, error in
            ...
            // We're done!
            Crashlytics.crashlytics().setCustomKeysAndValues([
                FirebaseCustomKeys.PACEStep: "Done",
                FirebaseCustomKeys.PACEStatus: "success"
            ])
